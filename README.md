...
`ptdata01` is provided with the package.
...
count of non-target responses provided to the argument `count_e`.

If you directly have frequencies of a target behavior, you can provide
it to the argument `freq` (or `freq_e` for a non-target behavior).
Because `count` and `time` contain more information, if they are already
provided, the priority is given to them and `freq` (or `freq_e`) is
ignored.
